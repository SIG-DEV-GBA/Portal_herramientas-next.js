...
// Types
import { ApiList, Filters, Ficha } from "@/apps/gestor-fichas/lib/types";
import { apiJSON } from "@/apps/gestor-fichas/lib/api";
import { PAGE_SIZES, fmtDate, pickFichasFilters } from "@/apps/gestor-fichas/lib/utils";

// Components
import FichaDetailModal from "./FichaDetailModal";
import { ConfirmModal, NotificationModal } from "@/shared/components/ui/Modal";
...
export default function FichasTableModern({ filters, onChange, onRecordCountChange }: Props) {
  ...
  const handleFichaUpdate = (updatedFicha: Ficha) => {
    // Actualizar la ficha en el estado local
    setData(prevData => ({
      ...prevData,
      items: prevData.items.map(item => 
        item.id === updatedFicha.id ? { ...item, ...updatedFicha } : item
      )
    }));
    
    // Actualizar la ficha seleccionada si es la misma
    if (selectedFicha && selectedFicha.id === updatedFicha.id) {
