...
import { pickFichasFilters, monthName } from "@/app/apps/gestor-fichas/lib/utils";
...
export default function TematicasDistribucionDonut({ filters }: { filters: Filters }) {
  ...
  useEffect(() => {
    let cancel = false;
    setLoading(true);
    apiJSON<RowApi[]>("/api/apps/gestor-fichas/stats/tematicas-distribucion", qs, { cache: "no-store" })
      .then((rows) => {
        if (cancel) return;
        const out: Row[] = (rows ?? [])
