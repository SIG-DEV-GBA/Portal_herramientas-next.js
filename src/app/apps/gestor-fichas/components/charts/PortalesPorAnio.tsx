...
export default function PortalesPorAnioChart({ filters }: { filters: Filters }) {
  ...
  // año + filtros compatibles con /api/apps/gestor-fichas/stats/portales (ambito, tramite_tipo, etc.)
  const query = useMemo(
  ...
  useEffect(() => {
    ...
    fetch(`/api/apps/gestor-fichas/stats/portales?${params.toString()}`, { cache: "no-store" })
